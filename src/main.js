/**
 * FarmSense — Main Application
 * Ties together all services, components, and charts
 */
...
async function loadWeatherData(lat, lon) {
  ...
  try {
    ...
    const [forecast, historical] = await Promise.all([
      ...
      fetchHistorical(lat, lon).catch(() => null), // Historical is optional
    ]);
...
function renderHistorical(forecast, historical) {
  ...
  if (!grid || !historical?.daily || !forecast?.daily) return;
